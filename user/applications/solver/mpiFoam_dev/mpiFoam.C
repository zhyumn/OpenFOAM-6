...
#include "createTime.H"
    if (SUPstream::parRun())
    {
        ...
        //Pout << "start=" << MemStart<0>::start << endl;
        //pslab=&slab;
        SUPstream::shared_data<SharedPointer<int>> s_a(SUPstream::node_manager);
        SharedPointer<int>& a = s_a();
        SUPstream::Sync();
        if (SUPstream::node_manager.rank == 0)
        {
            a = slab.alloc(sizeof(int));
            //Pout<<"a____1="<<*(int*)(a.offset+MemStart<0>::start)<<endl;
            *a = 100;
            //Pout<<"a____2="<<*(int*)(a.offset+MemStart<0>::start)<<endl;
        }
        SUPstream::Sync();
        Pout << "a=" << *a << endl;
        //Pout << "offset=" << a.offset << endl;
