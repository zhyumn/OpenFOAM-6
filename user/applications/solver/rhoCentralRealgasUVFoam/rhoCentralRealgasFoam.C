...
#include "createTimeControls.H"
    ...
    // Info<<"before 1st thermo correct"<<endl;
    // thermo.correct();
    turbulence->validate();
...
#include "readFluxScheme.H"
    ...
    while (runTime.run())
    {
        ...
        e.boundaryFieldRef() == rhoE.boundaryField()/rho.boundaryField() - 0.5 * magSqr(U.boundaryField());

        
        // Info<<"\n before phi estimate \n";
        // volScalarField::Boundary &TBf = T.boundaryFieldRef();
        // volScalarField::Boundary &heBf = rhoE.boundaryFieldRef();
        // volScalarField::Boundary &cBf = c.boundaryFieldRef();
        // volScalarField::Boundary &pBf = p.boundaryFieldRef();
        // //volVectorField::Boundary &UBf = U.boundaryFieldRef();
        // forAll(T.boundaryField(), patchi){
        //     fvPatchScalarField &pT = TBf[patchi];
        //     fvPatchScalarField &pE = heBf[patchi];
        //     fvPatchScalarField &pc = cBf[patchi];
        //     fvPatchScalarField &pp = pBf[patchi];
        //     //fvPatchVectorField &pU = UBf[patchi];
        //     forAll(pT, facei){
        //         if( (patchi == 1)){ //} & ((facei == 37 || facei == 38))){
        //             Info<<patchi<<"\t"<<facei<<"\t"<<pT[facei]<<"\t"<<pp[facei]<<"\t"<<pE[facei]<<"\t"<<pc[facei]<<endl; //"\t"<<pU[0][facei]<<"\t"<<pU[1][facei]<<endl;
        //         }
        //     }
        // }
...
#include "YEqn.H"
        ...
        //     forAll(pT, facei){
        //         if( (patchi == 1)){ //} & ((facei == 37 || facei == 38))){
        //             Info<<patchi<<"\t"<<facei<<"\t"<<pT[facei]<<"\t"<<pp[facei]<<"\t"<<pE[facei]<<"\t"<<pc[facei]<<"\t"<<pU[0][facei]<<"\t"<<pU[1][facei]<<endl;
        //         }
        //     }
        // }
        ...
        forAll(Y, i)
        {
            if (i != inertIndex && composition.active(i))
            {
                ...
                volScalarField &Yinert = Y[inertIndex];
                volScalarField &rhoYi = rhoY[i];
                volScalarField &rhoYinert = rhoY[inertIndex];

                Yi.correctBoundaryConditions();
                Yi.max(1e-5);
                Yi.min(1.0);

                Yinert.boundaryFieldRef() = 1.0 - Yi.boundaryField();
                Yinert.max(1e-5);
                Yinert.min(1.0);

                rhoYi.boundaryFieldRef() = rho.boundaryField() * Yi.boundaryField();
                rhoYinert.boundaryFieldRef() = rho.boundaryField() * Yinert.boundaryField();
        ...
        // Info<<"rho Bc \n";
        // Info<<rho.boundaryField()<<endl;
        // Info<<"e Bc \n";
        // Info<<e.boundaryField()<<endl;
        // Info<<"T Bc \n";
