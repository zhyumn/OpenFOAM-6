...
#include "PFTerms.H"
        ...
        // --- Solve density
        solve(fvm::ddt(rho) + fvc::div(phi) - massReg);
        
        volScalarField t1 = fvc::div(phi);

        // forAll(T,i){
        //     Info<<frac[i]<<"\t"<<massReg[i]<<"\t"<<temporary[i]<<endl;
        // }

        // --- Solve momentum
        solve(fvm::ddt(rhoU) + fvc::div(phiUp) - momReg);
...
#include "YEqn.H"
        ...
        volScalarField t2(
            ...
            mesh,
            dimensionedScalar("t2", dimMass/dimLength/dimTime/dimTime/dimTime, 1E-6)
            );
        ...
        solve(rhoEEqn - energyReg);
        //solve(rhoeEqn);
        // forAll(T,i){
        //     Info<<frac[i]<<"\t"<<massReg[i]<<"\t"<<t1[i]<<"\t"<<div_Rs[0][i]<<"\t"<<tY[0][i]<<"\t"<<energyReg[i]<<"\t"<<t2[i]<<endl;
        // }
