...
void test::predict(float u, float v, float x1, float x2)
{
    ...
    int ndims = 2;
    int64_t dims[] = {batch_size,4};
    float data[] = {u,v,x1,x2}; 
    int ndata = sizeof(float)*batch_size*4;
...
int main(){
    ...
    const char* saved_model_dir = "./TPX/";//test_model_64_64_32_ind_pbigger_1e-3/"; // Path of the model
    const char* tags = "serve"; // default model serving tag; can change in future
    int ntags = 1;

    test obj(saved_model_dir,tags,ntags);
    obj.predict(400,2000000,0.7156593087182554,0.2843406912817447);
