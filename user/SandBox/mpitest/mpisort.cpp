...
int main(void) {
    ...
    //bool ordered = false;
    sync0.sync();
    for (int i = 0; i < node_manager.rank * 100 + 10; i++) {
...
*/
    ...
    int save, found_i, insert_pos,index_disordered;
    bool found_disordered = false, success = false, ordered = false, insert = false, found_pos = false;
    while (!ordered) {
        if (found_disordered) {
            roommy.require_empty_room();
            int inext = lltest[index_disordered].next;
            if (inext >= 0 && lltest[index_disordered].data > lltest[inext].data) {
                save = lltest[inext].data;
                found_pos = lltest.remove(index_disordered);
            }
            ...
            if (save <= lltest[i].data) {
                ...
                for (; i != -1; i = lltest[i].next) {
                    ...
                    if (inext != -1 && save <= lltest[inext].data) {
                        insert_pos = i;
                        found_pos = false;
                        break;
                    }
