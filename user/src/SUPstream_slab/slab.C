...
#define ONE_USED_SLOT(slots, empty_slotmask) \
    (                                        \
        (                                    \
            ...
            ((~(slots) & (empty_slotmask)) - 1)) == SLOTS_ALL_ZERO)
...
namespace Foam
{
    ...
    void slab_chain::init(const size_t itemsize)
    {
        ...
        this->slabsize = (size_t)1 << (size_t)CLOG2(least_slabsize);
        this->itemcount = 64;
        ...
        this->pages_per_alloc = this->slabsize > pagesize ? this->slabsize : pagesize;
    ...
    SHAREDPOINTER slab_chain::alloc(MemPool &mempool)
    {
        ...
        else if (LIKELY((this->partial = this->empty) != sptr_NULL))
        {
            ...
            UNLIKELY(this->partial->refcount != 0) ? this->partial->refcount++ : this->partial->page->refcount++;
    ...
    void slab_chain::dump(FILE *const out)
    {
        ...
        for (size_t i = 0; i < 3; ++i)
        {
            ...
            for (row = 1; slab != sptr_NULL; slab = slab->next, ++row)
            {
                ...
                for (int k = 63; k >= 0; --k)
                {
                    fprintf(out, slab->slots & (SLOTS_FIRST << k) ? GREEN("1") : ((size_t)k >= this->itemcount ? GRAY("0") : RED("0")));
                }
    ...
    void slab_chain::report()
    {
        ...
        occupancy = total_used_slots + total_free_slots ? 100 * (float)total_used_slots / (total_used_slots + total_free_slots) : 0.0;
        std::cout << "Total"
                  << " " << total_nr_slabs << " " << total_used_slots << " " << total_free_slots << " " << occupancy << std::endl;
        std::cout << std::endl;
        //fprintf(out, "%8s %17llu %17llu %17llu %16.2f%%\n", "Total",
        //    total_nr_slabs, total_used_slots, total_free_slots, occupancy);
    }
