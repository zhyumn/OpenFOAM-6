...
#include "ISATVLEheRhoReactingThermo.H"

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

template<class BasicPsiThermo, class MixtureType>
void Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::calculate_init()
{
    ...
    //Nav added
    //scalarField& QdotCells = this->Qdot().primitiveFieldRef();
    //scalarField& reactRateCells = this->reactRate[i]().primitiveFieldRef();
        ...
        forAll(heList_, i)
        {
            forAll(TCells, celli)
            {
                ...
                heList_[i].primitiveFieldRef()[celli]
                    = this->speciesData()[i].Hs(pCells[celli], TCells[celli]);
            ...
            forAll(this->T_.boundaryField(), patchi)
            {
                ...
                if (pT.fixesValue())
                {
                    forAll(pT, facei)
                    {
                        ...
                        pheList[facei]
                            = this->speciesData()[i].Hs(pp[facei], pT[facei]);
                    }
                }
                else
                {
                    forAll(pT, facei)
                    {
                        ...
                        pDimix[facei]
                            = mixture_.Dimix(pp[facei], pT[facei], i);
                        pheList[facei]
                            = this->speciesData()[i].Hs(pp[facei], pT[facei]); //TODO check Hs
...
template<class BasicPsiThermo, class MixtureType>
void Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::calculate()
{
...
template<class BasicPsiThermo, class MixtureType>
Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::ISATVLEheRhoReactingThermo
(
    const fvMesh& mesh,
...
template<class BasicPsiThermo, class MixtureType>
void Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::correct()
{
    if (debug)
    {
        InfoInFunction << endl;
...
// ************************************************************************* //
template<class BasicPsiThermo, class MixtureType>
void Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::customChemistry(scalar temperature, scalar pressure, scalar density){
    setThermo(temperature, pressure, density);
    //Mw kg/kmol
    const typename MixtureType::thermoType& mixture_ =
                this->cellMixture(0);
    W_r[0] = mixture_[0].W();  //c12h26 
    W_r[1] = mixture_[1].W();  //O2
    W_r[2] = mixture_[2].W();  //co2
    W_r[3] = mixture_[3].W();  //co
    W_r[4] = mixture_[4].W();  //h2o
    W_r[5] = mixture_[5].W();  //N2
}

template<class BasicPsiThermo, class MixtureType>
void Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::setThermo(scalar temperature, scalar pressure, scalar density){
    T_r = temperature;
    P_r = pressure;
    rho_r = density;
    setRate();
...
template<class BasicPsiThermo, class MixtureType>
void Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::setY(double Y, int speciesLabel){
    Ygas_r[speciesLabel] = Y;
}

template<class BasicPsiThermo, class MixtureType>
void Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::setConc(){
    for (int i=0; i<5; i++){
        c_r[i] = rho_r*Ygas_r[i]/W_r[i];
    }
}

template<class BasicPsiThermo, class MixtureType>
void Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::setRate(){
    double A1;
    double R1, R2;
    ...
    phi_st = 1.0; //should be local equivalence ratio. NEED TO FIX

    //0 = c12h26. 1 = o2, 2 = co2, 3 = co, 4 = h2o, 5 = inert
    A1 = std::exp(t0 + t1*std::exp(t2*phi_st) + t3*std::tanh( (t4 + t5*phi_st)*T_r + t6));
    R1 = A1 * std::exp(-31944/(R_r*T_r)) * std::pow(c_r[0],0.25) * std::pow(c_r[1],1.25);
    R2 = 3.98*1E14*std::exp(-40000/(R_r*T_r))* std::pow(c_r[3],1.0)* std::pow(c_r[4],0.5)* std::pow(c_r[1],0.25) - 5.0*1E7*std::exp(-40000/(R_r*T_r))*std::pow(c_r[2],1.0);
...
template<class BasicPsiThermo, class MixtureType>
double Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::RR(int speciesLabel){
    //convert to SI and return. To Do.
    return reactRate_r[speciesLabel];
}

template<class BasicPsiThermo, class MixtureType>
double Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::Qdot(){
    double q = 0.0;
    const typename MixtureType::thermoType& mixture_ =
                this->cellMixture(0);
    for(int i = 0; i<5; i++) {
        q -= mixture_[i].Hc()*reactRate_r[i];
    }
    return q;
}

template<class BasicPsiThermo, class MixtureType>
double Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::gasPhaseRho(scalar pressure, scalar temperature){
    const typename MixtureType::thermoType& mixture_ = this->cellMixture(0);    
    return mixture_.rho(pressure,temperature);
}

template<class BasicPsiThermo, class MixtureType>
double Foam::ISATVLEheRhoReactingThermo<BasicPsiThermo, MixtureType>::gasPhaseY(label celli, label speciesLabel){
    const typename MixtureType::thermoType& mixture_ = this->cellMixture(celli);
}
