...
void Foam::ISATVLEheRhoThermo<BasicPsiThermo, MixtureType>::calculate_init()
{
    ...
    forAll(TCells, celli)
    {
        ...
        std::tie(he_temp, rhoCells[celli], vaporfracCells[celli],  rho_G_Cells[celli]) = mixture_.Erhovf_G_rhoY_ISAT(pCells[celli], TCells[celli], Y_temp);
    ...
    forAll(this->T_.boundaryField(), patchi)
    {
        ...
        if (pT.fixesValue())
        {
            forAll(pT, facei)
            {
                ...
                std::tie(he_temp, prho[facei], pvaporfrac[facei], prho_G[facei]) = mixture_.Erhovf_G_rhoY_ISAT(pp[facei], pT[facei], Y_temp);
            ...
            if (pT.fixesValue())
            {
                forAll(pT, facei)
                {
                    const typename MixtureType::thermoType &mixture_ =
                        this->patchFaceMixture(patchi, facei);
                    //std::tie(prho[facei], pvaporfrac[facei], psoundspeed[facei]) = mixture_.rhovfc_ISAT(pp[facei], pT[facei]);
                    std::tie(phe[facei], prho[facei], pvaporfrac[facei],  prho_G[facei]) = mixture_.Erhovf_G_rhoY_ISAT(pp[facei], pT[facei], Y_temp);
                    //pZ[facei] = mixture_.Z(pp[facei], pT[facei]);
                    for (int i = 0; i < Y_temp.size(); i++)
                    {
                        Y_G_List_[i].boundaryFieldRef()[patchi][facei] = Y_temp[i];
