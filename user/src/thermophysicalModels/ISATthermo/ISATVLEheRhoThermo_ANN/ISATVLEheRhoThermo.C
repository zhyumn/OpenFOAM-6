...
void Foam::ISATVLEheRhoThermo<BasicPsiThermo, MixtureType>::calculate()
{
    ...
    if (!boundary_flag)
    {
        ...
        else if (scheme_ == "conservativeFlux")
        {
            ...
            //     forAll()
            // 	    Z[i] = Y[i][celli]*Wmix[celli]/this->speciesData()[i].W();

            //     std::tie(TCells[celli], pCells[celli], vaporfracCells[celli], soundspeedCells[celli]) = ANN.predict(hCells[celli]*Wmix[celli], Wmix[celli]/rhoCells[celli], TCells[celli], pCells[celli]);
            // }
        }
        else if (scheme_ == "mix")
        {
...

{
    ...
    RunOpts  = NULL;
    char* saved_model_dir = "/scratch.nike/srini237/OpenFOAM/temp/OpenFOAM-6/user/src/thermophysicalModels/ISATthermo/ISATVLEheRhoThermo_ANN/ANN/test_model_64_64_32_ind_pbigger_1e-3/"; // Path of the model
    char* tags = "serve"; // default model serving tag; can change in future
    int ntags = 1;
    ANN_load(saved_model_dir,tags,ntags); 
