...
namespace MPI_Shared
{
    ...
    SHAREDPOINTER slab_chain::alloc()
    {
        assert(is_valid());
        ...
        if (LIKELY(this->partial != sptr_NULL)) {
            ...
            this->partial = pmempool->get_page(this->pages_per_alloc);
    ...
    void slab_chain::free(SHAREDPOINTER addr)
    {
        assert(is_valid());
        ...
        if (UNLIKELY(slab->slots == SLOTS_ALL_ZERO)) {
            ...
            if (UNLIKELY(slab->refcount == 1 || (slab->refcount == 0 &&
                ...
                if (UNLIKELY(found_head && (this->empty = this->empty->next) != sptr_NULL))
                    this->empty->prev = sptr_NULL;
                if (UNLIKELY(pmempool->free_page(page, this->pages_per_alloc) == -1))
                    perror("free_page");
    ...
    void slab_chain::destroy()
    {
        ...
        if (LIKELY(pages_head != sptr_NULL)) {
            ...
            do {
                ...
                if (UNLIKELY(pmempool->free_page(target, this->pages_per_alloc) == -1))
                    perror("free_page");
