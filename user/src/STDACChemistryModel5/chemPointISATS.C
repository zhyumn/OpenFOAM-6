...
Foam::scalar Foam::chemPointISATS<CompType, ThermoType>::tolerance_;

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

template <class CompType, class ThermoType>
void Foam::chemPointISATS<CompType, ThermoType>::qrDecompose(
    const label nCols,
...
template <class CompType, class ThermoType>
void Foam::chemPointISATS<CompType, ThermoType>::qrUpdate(
    scalarSquareMatrix &R,
    const label n,
...
template <class CompType, class ThermoType>
void Foam::chemPointISATS<CompType, ThermoType>::rotate(
    scalarSquareMatrix &R,
    const label i,
    const scalar a,
    const scalar b,
...
Foam::chemPointISATS<CompType, ThermoType>::chemPointISATS(
    ...
    binaryNodeS<CompType, ThermoType> *node)
...
template <class CompType, class ThermoType>
bool Foam::chemPointISATS<CompType, ThermoType>::inEOA(const scalarField &phiq)
{
...
template <class CompType, class ThermoType>
bool Foam::chemPointISATS<CompType, ThermoType>::checkSolution(
...
template <class CompType, class ThermoType>
bool Foam::chemPointISATS<CompType, ThermoType>::grow(const scalarField &phiq)
{
    scalarField dphi(phiq - phi());
...
template <class CompType, class ThermoType>
void Foam::chemPointISATS<CompType, ThermoType>::increaseNumRetrieve()
{
    this->numRetrieve_++;
}

template <class CompType, class ThermoType>
void Foam::chemPointISATS<CompType, ThermoType>::resetNumRetrieve()
{
    this->numRetrieve_ = 0;
}

template <class CompType, class ThermoType>
void Foam::chemPointISATS<CompType, ThermoType>::increaseNLifeTime()
{
    this->nLifeTime_++;
}

template <class CompType, class ThermoType>
Foam::label Foam::chemPointISATS<CompType, ThermoType>::
...
template <class CompType, class ThermoType>
void Foam::chemPointISATS<CompType, ThermoType>::retrieve(const scalarField &phiq, scalarField &Rphiq)
{
    label nEqns = this->chemistry_.nEqns(); // Species, T, p
    bool mechRedActive = this->chemistry_.mechRed()->active();
    Rphiq = this->Rphi();
    scalarField dphi(phiq - this->phi());
    const scalarSquareMatrix &gradientsMatrix = this->A();
    List<label> &completeToSimplified(this->completeToSimplifiedIndex());
    ...
    for (label i = 0; i < nEqns - nAdditionalEqns_; i++)
    {
        if (mechRedActive)
        {
            ...
            if (si != -1)
            {
                ...
                Rphiq[i] +=
                    gradientsMatrix(si, this->nActiveSpecies()) * dphi[nEqns - 2];
                Rphiq[i] +=
                    gradientsMatrix(si, this->nActiveSpecies() + 1) * dphi[nEqns - 1];

                if (this->variableTimeStep())
                {
                    Rphiq[i] +=
                        gradientsMatrix(si, this->nActiveSpecies() + 2) * dphi[nEqns];
                }
