    ...
    const DeltaTType &deltaT)
{
    ...
    if (tabulation_->active())
    {
        ...
        for (int i = 0; i < nloop; i++)
        {
            ...
            for (int celli = i * batch_size; celli < last_i; celli++)
            {
                tmp.celli = celli;
                tmp.rho = rho[celli];
                tmp.p = p[celli];
